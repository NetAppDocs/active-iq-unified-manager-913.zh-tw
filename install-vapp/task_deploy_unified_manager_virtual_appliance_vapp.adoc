...
+
** 確認「發行者」區段的詳細資料。「Entrust Code Signing - OCS2（Trusted Certificate）*（Entrust程式碼簽署- OCS2（信任的憑證）*）」訊息可確認下載項目的完整性 `OVA` 檔案：
 +
如果您看到訊息 * 信任程式碼簽署 - OVCS2 （無效憑證） * 、請將 VMware vCenter Server 升級至 7.0U3E 或更新版本。
