...
* *狀態*
+
顯示成員Aggregate的目前狀態。狀態可以是「Critical」（重大）（image:../media/sev_critical_um60.png[""]）、錯誤（image:../media/sev_error_um60.png[""]）、警告（image:../media/sev_warning_um60.png[""]）或正常（image:../media/sev_normal_um60.png[""]）。
...
** 混合式RAID：該集合體包含不同RAID類型（RAID0、RAID4、RAID-DP及RAID-TEC 支援）的RAID群組。
依預設、此欄為隱藏欄。
