...
. 在*編輯叢集*對話方塊中、視需要修改值。
 +
如果您已修改新增至 Unified Manager 之叢集的詳細資料、您可以根據 ONTAP 版本、檢視相互 TLS 通訊的憑證詳細資料。如需ONTAP 更多關於更新版本的資訊、請參閱 link:../storage-mgmt/task_add_clusters.html["相互TLS通訊的憑證"]。
 +
您可以按一下 * 憑證詳細資料 * 來檢視憑證詳細資料。如果憑證已過期、請按一下*重新產生*按鈕、以納入新的憑證。
. 按一下*提交*。
