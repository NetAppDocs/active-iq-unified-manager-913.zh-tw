...
定期檢查具有授權的叢集、以進行反勒索軟體監控。驗證儲存VM是否僅支援這類叢集中的NFS或SMB磁碟區。
 a| 
將儲存VM放入 `learning` 防勒索軟體監控模式
 a| 
Unified Manager會將勒索軟體監控設定為 `learning` 透過叢集管理主控台說明儲存VM的狀態。在儲存VM上建立的所有新磁碟區上進行的反勒索軟體監控、會自動移至學習模式。透過這項功能、ONTAP 我們可以瞭解磁碟區上的活動模式、並偵測潛在惡意攻擊所造成的異常狀況。
...
定期檢查具有授權的叢集、以進行反勒索軟體監控。驗證磁碟區是否僅支援這類叢集中的NFS或SMB服務。
 a| 
將磁碟區放入 `learning` 防勒索軟體監控模式
 a| 
Unified Manager會將勒索軟體監控設定為 `learning` 透過叢集管理主控台說明磁碟區的狀態。透過這項功能、ONTAP 我們可以瞭解磁碟區上的活動模式、並偵測潛在惡意攻擊所造成的異常狀況。
...
定期檢查具有授權的叢集、以進行反勒索軟體監控。偵測磁碟區是否位於 `learning` 反勒索軟體監控模式超過45天、並決定將其置於作用中模式的可能性。
 a| 
將磁碟區放入 `active` 防勒索軟體監控模式
 a| 
Unified Manager會將勒索軟體監控設定為 `active` 透過叢集管理主控台在磁碟區上執行。透過這項功能、ONTAP 我們可以瞭解磁碟區上的活動模式、偵測潛在惡意攻擊所造成的異常狀況、並建立資料保護行動警示。
