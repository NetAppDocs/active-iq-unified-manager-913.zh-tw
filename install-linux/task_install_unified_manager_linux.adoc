...
|===
. 如果安裝的 MySQL 版本早於 MySQL 8.0.32 Community Edition 、請輸入下列命令將其解除安裝：
+
...
. 瀏覽至您下載安裝的目錄 `.zip` 歸檔並展開Unified Manager產品組合：
+
...
. 做為root使用者、或使用 `sudo`下、執行下列命令以安裝軟體：
+
...
[NOTE]
====
請勿使用替代命令（例如 `rpm -ivh`）。若要在Red Hat Enterprise Linux或CentOS系統上成功安裝Unified Manager、必須將所有Unified Manager檔案及相關檔案以特定順序安裝至由自動強制執行的特定目錄結構 `yum install netapp-um<version>.x86_64.rpm` 命令。
